...
st.markdown('<hr style="border:none;border-top:1px solid var(--border);margin:2rem 0 1.5rem;">', unsafe_allow_html=True)
st.markdown('<div class="ete-section-label"><div class="ete-section-dot"></div><span>Resumo — Sopradores (copiar e colar)</span></div>', unsafe_allow_html=True)
...
# Suporte a múltiplas chaves — GOOGLE_API_KEY, GOOGLE_API_KEY_2, GOOGLE_API_KEY_3...
...
GOOGLE_API_KEYS_MICRO = _carregar_chaves_gemini()
GOOGLE_API_KEY_MICRO  = GOOGLE_API_KEYS_MICRO[0] if GOOGLE_API_KEYS_MICRO else ""

# ──────────────────────────────────────────────────────────────────────────────
# CONTADOR DE COTA GEMINI
# ──────────────────────────────────────────────────────────────────────────────
_LIMITE_DIARIO   = 1500
_LIMITE_POR_MIN  = 15
_COTA_FILE       = "/tmp/gemini_cota.json"

def _cota_carregar() -> dict:
    """Lê o arquivo de cota. Reseta se já passou das 04:00 BRT (reset do Google)."""
    import json, os
    from datetime import datetime, timezone, timedelta
    BRT = timezone(timedelta(hours=-3))
    agora = datetime.now(BRT)
    # Reset às 04:00 BRT (meia-noite PT)
...
def _cota_incrementar():
    """Registra uma requisição feita. Retorna (total_dia, req_ultimo_min)."""
    from datetime import datetime, timezone, timedelta
    ...
    # Mantém só os timestamps do último minuto
    hist = dados.get("historico_min", [])
    hist.append(agora)
...
def _cota_status() -> dict:
    """Retorna status atual da cota sem incrementar."""
    from datetime import datetime, timezone, timedelta
    BRT = timezone(timedelta(hours=-3))
    agora = datetime.now(BRT).timestamp()
    dados = _cota_carregar()
    hist = [t for t in dados.get("historico_min", []) if agora - t < 60]
    n_chaves = len(GOOGLE_API_KEYS_MICRO)
    return {
        "total_dia": dados.get("total_dia", 0),
        "limite_dia": _LIMITE_DIARIO * max(n_chaves, 1),
        "req_min": len(hist),
        "limite_min": _LIMITE_POR_MIN * max(n_chaves, 1),
        "n_chaves": n_chaves,
    }

def _cota_widget():
    """Exibe card com uso da cota Gemini."""
    from datetime import datetime, timezone, timedelta
    BRT = timezone(timedelta(hours=-3))
    agora = datetime.now(BRT)
    ...
    horas_reset = int((reset - agora).total_seconds() // 3600)
    mins_reset  = int(((reset - agora).total_seconds() % 3600) // 60)
    ...
    with st.expander(f"{cor} Cota Gemini — {s['total_dia']}/{s['limite_dia']} req hoje", expanded=False):
        ...
        col1.metric("Usadas hoje", s["total_dia"], delta=f"/{s['limite_dia']}")
        col2.metric("Req/min agora", s["req_min"], delta=f"/{s['limite_min']} limite")
        col3.metric("Chaves ativas", s["n_chaves"])
        st.progress(min(pct, 1.0))
        st.caption(f"🔄 Reset em {horas_reset}h {mins_reset}min (04:00 BRT) · Dados aproximados por sessão")

# ──────────────────────────────────────────────────────────────────────────────
# PROMPT ESTRUTURADO
# ──────────────────────────────────────────────────────────────────────────────
SYSTEM_PROMPT_MICRO = """Você é um especialista em microbiologia de lodos ativados segundo a norma CETESB L1.025.

REGRA CRÍTICA: Você SEMPRE deve retornar pelo menos 1 organismo ou estrutura. NUNCA retorne "organismos" vazio. Mesmo em imagens de baixa qualidade, identifique flocos, partículas ou estruturas visíveis.

Chaves válidas (use EXATAMENTE uma delas):
flagelados, flagelados_rizopodes, ciliados_pedunculados, ciliados_livres, arcella, aspidisca, trachelophyllum, vorticella_microstoma, aelosoma, rotiferos, filamentos, nematoides, rizopodes_amebas, flocos_bons, flocos_dispersos, cianobacterias, protozoa_livre

Estratégia para imagens difíceis:
- Imagem turva/escura → identifique "flocos_dispersos" com confiança 0.4
- Partículas visíveis sem forma clara → identifique "flocos_bons" ou "flocos_dispersos"
- Estruturas filamentosas → identifique "filamentos"
- Organismos móveis/cilíndricos → identifique "flagelados" ou "ciliados_livres"
- Em caso de dúvida, SEMPRE escolha a chave mais próxima com confiança baixa (0.3–0.5)

Retorne SOMENTE JSON puro, sem markdown, sem texto extra:
{"organismos":[{"chave":"chave_valida","nome":"Nome científico ou comum","grupo":"Grupo taxonômico","descricao":"Breve observação","confianca":0.7}],"qualidade_imagem":"boa|regular|ruim","nitidez_score":0.6,"observacoes_gerais":"observação geral"}

LEMBRE: organismos[] NUNCA pode ser vazio. Sempre inclua ao menos 1 entrada."""
...
def _extrair_frames_video(video_bytes: bytes, max_frames: int = 2) -> list:
    """
    Extrai exatamente 2 frames do vídeo usando ffmpeg.
    Distribuídos em 33% e 66% da duração para pegar momentos distintos.

    Args:
        video_bytes: Bytes do arquivo de vídeo
        max_frames: Número de frames a extrair (fixo em 2)

    Returns:
        Lista de strings base64 dos frames extraídos
    """
    frames_b64 = []
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Extrai exatamente 2 frames: em 33% e 66% do vídeo
        timestamps = [duration * 0.33, duration * 0.66]

        for i, t in enumerate(timestamps):
            frame_path = os.path.join(tmpdir, f"frame_{i:02d}.jpg")
            subprocess.run(
                ["ffmpeg", "-ss", str(t), "-i", video_path,
                 "-vframes", "1", "-vf", "scale=512:-1", "-q:v", "6",
                 frame_path, "-y"],
                capture_output=True
            )
...
def _comprimir_imagem_b64(b64_str: str, max_lado: int = 512, qualidade: int = 60) -> str:
    """
    Comprime uma imagem base64 para reduzir consumo de tokens na API.
    Redimensiona para no máximo max_lado pixels e aplica compressão JPEG.
    Economiza ~60-80% dos tokens de imagem sem perder capacidade de análise.
    """
    try:
        ...
        # Recomprime com qualidade reduzida
        buf = _io.BytesIO()
        img.save(buf, format="JPEG", quality=qualidade, optimize=True)
        return base64.b64encode(buf.getvalue()).decode()
    except Exception:
        return b64_str  # fallback: retorna original se PIL falhar
...
def _chamar_gemini_micro(frames_b64: list, params_operacionais: dict, api_key: str) -> dict:
    ...
    Implementa retry automático com backoff para erros 503 (alta demanda).
    """
    import time

    # Comprime imagens antes de enviar — economiza tokens da API
    # Comprime mantendo qualidade suficiente para análise microscópica
    frames_b64 = [_comprimir_imagem_b64(f, max_lado=1024, qualidade=85) for f in frames_b64]
    ...
    parts.append({"text": f"""Analise CUIDADOSAMENTE estes frames de microscópio de lodo ativado de ETE.{ctx_params}
IMPORTANTE: Você DEVE identificar pelo menos 1 organismo ou estrutura. Se a imagem for de baixa qualidade, identifique o que for visível (flocos, partículas, filamentos) com confiança baixa.
Retorne APENAS JSON puro conforme instruído."""})

    payload = {
        "system_instruction": {"parts": [{"text": SYSTEM_PROMPT_MICRO}]},
    ...
    # Lista de chaves disponíveis — tenta cada uma em caso de 429
    chaves_disponiveis = GOOGLE_API_KEYS_MICRO if GOOGLE_API_KEYS_MICRO else [api_key]

    ESPERA_BASE = 5
    MAX_RETRIES_503 = 3  # tentativas extras por erros de servidor (503/timeout)

    resp = None
    chaves_esgotadas = set()   # índices das chaves com 429 confirmado
    chave_idx = 0              # começa sempre pela primeira
    retries_servidor = 0

    while True:
        # Avança para a próxima chave não-esgotada
        tentativas_chave = 0
        while chave_idx in chaves_esgotadas:
            chave_idx = (chave_idx + 1) % len(chaves_disponiveis)
            tentativas_chave += 1
            if tentativas_chave >= len(chaves_disponiveis):
                # Todas as chaves esgotadas
                n_chaves = len(chaves_disponiveis)
                raise RuntimeError(
                    f"Cota da API Google esgotada em todas as {n_chaves} chave(s) configurada(s). "
                    f"Aguarde alguns minutos ou adicione mais chaves (GOOGLE_API_KEY_2, _3...) nos Secrets do Streamlit."
                )

        chave_atual = chaves_disponiveis[chave_idx]
        url = url_base + chave_atual
        ...
        try:
            ...
            if resp.status_code == 429:
                # Loga o erro real pra debug
                try:
                    erro_json = resp.json()
                    erro_msg = str(erro_json).lower()
                    print(f"[DEBUG 429] {erro_json}")
                except Exception:
                    erro_msg = resp.text.lower()
                    print(f"[DEBUG 429 text] {resp.text[:300]}")

                is_cota_diaria = "daily" in erro_msg or "per_day" in erro_msg

                if not is_cota_diaria:
                    raise RuntimeError(
                        "⏳ Rate limit atingido (muitas requisições por minuto). "
                        "Aguarde 1 minuto e tente novamente."
                    )
                else:
                    chaves_esgotadas.add(chave_idx)
                    chaves_restantes = len(chaves_disponiveis) - len(chaves_esgotadas)
                    if chaves_restantes > 0:
                        chave_idx = (chave_idx + 1) % len(chaves_disponiveis)
                        st.toast(f"⚠️ Cota diária da chave {chave_idx} esgotada — tentando próxima ({chaves_restantes} restante(s))...")
                        time.sleep(2)
                        continue
                    else:
                        n_chaves = len(chaves_disponiveis)
            ...
            if resp.status_code == 503:
                retries_servidor += 1
                if retries_servidor <= MAX_RETRIES_503:
                    espera = ESPERA_BASE * retries_servidor
                    st.toast(f"Servidor ocupado, aguardando {espera}s... (tentativa {retries_servidor}/{MAX_RETRIES_503})")
                    time.sleep(espera)
                    continue
                resp.raise_for_status()
            ...
            retries_servidor += 1
            if retries_servidor <= MAX_RETRIES_503:
                st.toast(f"Timeout, tentando novamente... ({retries_servidor}/{MAX_RETRIES_503})")
...
def aplicar_regras_cetesb(organismos: list) -> dict:
    ...
    do lodo com base em regras da CETESB L1.025 — versão balanceada.

    Lógica: pontuação ponderada por confiança.
      - Organismos bons somam pontos positivos
      - Organismos ruins somam pontos negativos
      - O resultado final considera o BALANÇO, não apenas a presença de ruim
    """
    if not organismos:
        return {
    ...
    # Mapa de confiança por chave (usa o maior se vier mais de uma vez)
    confianca_map = {}
    for org in organismos:
        ch = org.get("chave", "")
        conf = org.get("confianca", 0.5)
        confianca_map[ch] = max(confianca_map.get(ch, 0), conf)

    # ── Regras absolutas (independem do balanço) ──────────────────────────────

    # Cianobactérias: sempre ruim, risco de toxicidade real
    ...
    # Lodo totalmente disperso: sempre ruim
    if "flocos_dispersos" in chaves and "flocos_bons" not in chaves:
        return {
            "qualidade": "ruim",
            "descricao": "Lodo disperso (pin-point) sem flocos bons — má sedimentação.",
            "cor": COR_SEMAFORO["vermelho"],
            "icon": "",
            "acoes_recomendadas": [
    ...
    PESO = {
        ...
        "filamentos":            -2,   # era -999, agora -2 (considera o contexto)
    }

    score = 0.0
    acoes = []

    for ch, conf in confianca_map.items():
        peso = PESO.get(ch, 0)
        score += peso * conf  # pondera pela confiança da IA

    # Acoes específicas conforme organismos presentes
    if "filamentos" in chaves:
        acoes.append("Monitorar IVL — filamentos presentes mas podem não indicar bulking severo.")
        acoes.append("Verificar OD no tanque e sobrecarga orgânica.")
    if "flagelados" in chaves:
        acoes.append("Verificar OD no tanque de aeração — flagelados indicam deficiência.")
    if "vorticella_microstoma" in chaves:
        acoes.append("Investigar causa: sobrecarga, tóxicos ou aeração insuficiente.")
    if "aelosoma" in chaves:
        acoes.append("OD possivelmente elevado (> 6 mg/L) — considerar reduzir aeração.")
    if "flocos_dispersos" in chaves:
        acoes.append("Avaliar θc e presença de tóxicos no afluente.")
    ...
    if score >= 3:
        return {
            ...
            "icon": "",
            "acoes_recomendadas": acoes or ["Manter parâmetros operacionais atuais."]
        }
    elif score >= 0:
        return {
            "qualidade": "regular",
            "descricao": "Sistema em equilíbrio com organismos mistos — monitorar evolução.",
            "cor": COR_SEMAFORO["laranja"],
            "icon": "",
            "acoes_recomendadas": acoes or [
                "Monitorar organismos nas próximas análises.",
        ...
        return {
            "qualidade": "ruim",
            "descricao": "Organismos indicadores de problema — verificar operação do sistema.",
            "cor": COR_SEMAFORO["vermelho"],
            "icon": "",
            "acoes_recomendadas": acoes or [
                "Verificar OD, carga orgânica e idade do lodo.",
            ]
        }
    else:
        return {
            "qualidade": "critico",
            "descricao": "Múltiplos indicadores negativos — sistema com problemas sérios.",
            "cor": COR_SEMAFORO["vermelho"],
            "icon": "",
            "acoes_recomendadas": acoes or [
                "Acionar equipe técnica para revisão completa do processo.",
            ]
        }
...
def render_microbiologia():
    st.markdown('<hr style="border:none;border-top:1px solid var(--border);margin:2rem 0 1.5rem;">', unsafe_allow_html=True)
    st.markdown('<div class="ete-section-label"><div class="ete-section-dot"></div><span>Microbiologia do Lodo — Analise por IA (CETESB L1.025)</span></div>', unsafe_allow_html=True)
    ...
    st.info("Analise baseada em IA + regras da CETESB — Os resultados combinam visão computacional com a Tabela 6 da Norma L1.025.")

    # --- Widget de cota da API ---
    _cota_widget()
    ...
    if params_filtrados:
        st.caption("Parametros do último registro (usados como contexto para a IA):")
        cols_p = st.columns(len(params_filtrados))
        for i, (k, v) in enumerate(params_filtrados.items()):
            cols_p[i].metric(k, v)

    # --- Modo de upload ---
    st.subheader("Upload de Imagens ou Video do Microscopio")

    modo = st.radio(
        "Como quer enviar?",
        ["Fotos (JPG/PNG) — recomendado", "Video (MP4/MOV) — requer ffmpeg"],
        horizontal=True
    )
    ...
    if "Fotos" in modo:
        ...
        if imagens:
            ...
            # Usa as 2 primeiras imagens diretamente — sem filtro de nitidez
            frames_b64 = [base64.b64encode(b).decode() for b in todas_bytes[:2]]

            st.caption(f"{len(frames_b64)} imagem(ns) carregada(s):")
            cols_prev = st.columns(min(len(frames_b64), 2))
            for i, b64 in enumerate(frames_b64):
                cols_prev[i % 2].image(base64.b64decode(b64), caption=f"Imagem {i+1}", use_container_width=True)
        ...
        if analisar:
            ...
            with st.status("Analisando frames selecionados...", expanded=True) as status_micro:
                try:
                    ...
                    # Se a IA retornou lista vazia mesmo com status 200, avisa mas não inventa resultado
                    if not resultado_unico.get("organismos"):
                        st.warning(
                            "A IA não identificou nenhum organismo nesta imagem. "
                            "Tente com foto de melhor qualidade (mais iluminação e foco)."
                        )
                        resultado_unico["organismos"] = []
                        resultado_unico["qualidade_imagem"] = resultado_unico.get("qualidade_imagem", "ruim")
                        resultado_unico["observacoes_gerais"] = (
                            resultado_unico.get("observacoes_gerais") or
                            "Nenhum organismo identificado. Tente com imagem de melhor qualidade."
                        )

                    resultados_por_frame = [resultado_unico]
                    ...
                    msg = str(e)
                    if "Cota da API Google esgotada" in msg:
                        st.warning(
                            "⚠️ **Limite gratuito da API Google atingido.**\n\n"
                            "A análise por IA não está disponível agora — a cota diária foi esgotada. "
                            "Isso é uma limitação do plano gratuito do Google Gemini (1.500 requisições/dia).\n\n"
                            "**O que fazer:**\n"
                            "- Aguarde o reset às **04:00 BRT** (meia-noite horário do Pacífico)\n"
                            "- Ou adicione mais chaves nos Secrets do Streamlit (`GOOGLE_API_KEY_2`, `_3`...)\n\n"
                            "A análise manual continua disponível normalmente."
                        )
                    elif "Rate limit" in msg:
                        st.warning(
                            "⏳ **Muitas requisições por minuto.**\n\n"
                            "O Google limitou temporariamente o acesso. Aguarde **1 minuto** e tente novamente.\n\n"
                            "Isso não consome sua cota diária — é só uma pausa temporária."
                        )
                    else:
                        st.error(f"Erro: {msg}")
                    st.stop()
    ...
    # Avisos de qualidade de imagem (não-bloqueantes)
    if qualidade == "ruim":
        st.info("💡 Imagem de qualidade básica — resultados podem ter confiança menor. A análise foi realizada com o que estava disponível.")
    elif qualidade == "regular":
        st.info("Qualidade das imagens regular.")

    if obs_gerais:
        st.caption(obs_gerais)

    if not organismos:
        st.warning("Nenhum microrganismo identificado com as imagens fornecidas.")
        if st.button("Limpar resultado e tentar novamente"):
    ...
    m2.metric("Confianca Media", f"{confianca_med * 100:.0f}%" if confianca_med else "—")
    ...
    st.markdown(
        f"""<div style="background:{nivel_cor};border-radius:10px;padding:16px 20px;margin-bottom:12px;color:white;">
            <div style="font-size:18px;font-weight:700;letter-spacing:-0.02em">Qualidade: {nivel_qual}</div>
            <div style="font-size:13px;margin-top:6px;opacity:0.92">{nivel_desc}</div>
            <div style="font-size:11px;margin-top:8px;opacity:0.75;font-family:monospace">
                Analise baseada em IA + regras da CETESB L1.025
            </div>
        </div>""",
        unsafe_allow_html=True
    )
    ...
    if acoes:
        with st.expander("Acoes Recomendadas", expanded=True):
            for acao in acoes:
                st.markdown(f"• {acao}")
    ...
    st.subheader("Diagnostico do Processo (CETESB L1.025 — Tabela 6)")

    chaves_unicas = list({org.get("chave","") for org in organismos if org.get("chave","") in MICRO_TABELA6})

    # Se qualidade geral não é ruim/critico, rebaixa filamentos de erro para aviso
    qualidade_geral = nivel_qual.lower()
    def _semaforo_ajustado(chave, semaforo_original):
        if chave == "filamentos" and semaforo_original == "vermelho" and qualidade_geral not in ("ruim", "critico"):
            return "laranja"
        return semaforo_original

    diag_vermelho = [MICRO_TABELA6[c] for c in chaves_unicas if _semaforo_ajustado(c, MICRO_TABELA6[c]["semaforo"]) == "vermelho"]
    diag_laranja  = [MICRO_TABELA6[c] for c in chaves_unicas if _semaforo_ajustado(c, MICRO_TABELA6[c]["semaforo"]) == "laranja"]
    diag_verde    = [MICRO_TABELA6[c] for c in chaves_unicas if _semaforo_ajustado(c, MICRO_TABELA6[c]["semaforo"]) == "verde"]

    for d in diag_vermelho:
        st.error(f"**{d['condicao']}**\n\n→ {d['recomendacao']}")
    for d in diag_laranja:
        st.warning(f"**{d['condicao']}**\n\n→ {d['recomendacao']}")
    for d in diag_verde:
        st.success(f"**{d['condicao']}**")

    # ── Resumo copiável ──
    st.subheader("Resumo para WhatsApp / Relatorio")
    linhas = ["Analise Microbiologica do Lodo (CETESB L1.025):", ""]
    linhas.append(f"Qualidade do processo: {nivel_qual}")
    linhas.append(f"Confianca da analise: {confianca_med*100:.0f}%" if confianca_med else "")
    linhas.append("")
    linhas.append("Microrganismos identificados:")
    for org in organismos:
        conf_txt = f" (confianca: {org.get('confianca',0)*100:.0f}%)"
    ...
    for d in diag_vermelho + diag_laranja + diag_verde:
        linhas.append(f"{d['condicao']}")
    if diag_vermelho:
        linhas.append("")
        linhas.append("Acoes urgentes:")
        for d in diag_vermelho:
            linhas.append(f"• {d['recomendacao']}")
    if acoes:
        linhas.append("")
        linhas.append("Acoes recomendadas:")
        for acao in acoes:
            linhas.append(f"• {acao}")
    linhas.append("")
    linhas.append("Analise baseada em IA + regras da CETESB L1.025")
    ...
    if st.button("Limpar e analisar novamente"):
        del st.session_state["micro_resultado"]
        st.rerun()
